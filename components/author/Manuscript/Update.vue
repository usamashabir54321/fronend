<template>
	...
	<div class="settper_container" id="m_script_f_id">
		<div class="row">
			...
			<div id="settper_main_id" class="col s11 m10 l8">
				...
				<div id="settper_2" class="animate__animated animate__zoomIn" v-show="stteperPgIndx == 2">
					<form @submit.prevent="articleStep2Post">
						...
						<div class="settper_ftr">
							...
							<button type="submit" class="btn waves-effect waves-light amber darken-4 mr-2">Save</button>
							<button v-if="docFileArr.doc_file_n && allFigFiles.length > 0" @click="stteperPgIndx += 1" type="button" class="btn btn-info">Next <i class="material-icons right">arrow_forward</i></button>
						</div>
					</form>
				</div>
				...
				<div id="settper_4" class="animate__animated animate__zoomIn" v-show="stteperPgIndx == 4">
					...
					<div v-if="allIncRevwr.length > 0 || allExcRevwr.length > 0">
					...
					<form v-if="!allIncRevwr.length > 0 || !allExcRevwr.length > 0" @submit.prevent="articleStep4Post">
				...
				<div id="settper_5" class="animate__animated animate__zoomIn" v-show="stteperPgIndx == 5">
					<form @submit.prevent="articleStep5Post">
						...
						<div class="settper_ftr">
							<button @click="stteperPgIndx -= 1;" class="btn cyan waves-effect waves-light mr-2" type="button"><i class="material-icons left">arrow_back</i> Back</button>
							<button type="submit" class="btn waves-effect waves-light amber darken-4 mr-2">Save</button>
							<button @click="showViewComp('update')" type="button" class="btn btn-info">View information / metadata</button>
						</div>
					</form>
				</div>
...
<script>
	...
	export default {
		...
		mounted () {
			...
			const keyArr = this.allRow.keywords.split(",");
			for (var i = 0; i < keyArr.length; i++) {
				this.allKewords.push(keyArr[i]);
			}
			this.aStep2Data.a_no_tables = this.allRow.a_no_tables;
			this.aStep2Data.a_no_figures = this.allRow.a_no_figures;

			this.aStep5Data.no_conf = this.allRow.no_conf;
			if (this.allRow.con_typ_aff) this.aStep5Data.accept_con = 1;
			this.aStep5Data.con_typ_aff = this.allRow.con_typ_aff;
  			...
  			setTimeout(() => {
  				...
  				this.getJournalSecs();
  				this.getJSecIssues();
  				this.getIssueArtcleTyp();
  			},1000);
  			setTimeout(() => {
				this.getAllMyFiles(this.allRow.id);
				this.getAllAddFiles(this.allRow.id);
